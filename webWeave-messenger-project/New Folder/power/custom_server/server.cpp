...
void CustomSocket::coreListener(					// define core listener method 
	...
	if (state<1) throw std::runtime_error("Socket not build properly! Error at listening");// check type must be zero for client socket
    	...
    	if (bytesRead==-1){						// if failed to read message 
        	if (infoLevel>0) std::cerr<<"Failed to receive message from client at "<<inet_ntoa(address.sin_addr)<<":"<<ntohs(address.sin_port)<<std::endl;// print error info 
	...
	std::string receivedMessage(buffer,bytesRead);			// store message readed as string 
    	std::cout<<"Message received from client: "<<receivedMessage<<std::endl;// make print readed message from client 
...
bool Server::startListenAt(						// define connection method (Listening to client)
	...
	if (listen(serviceSocket.customSocket,1)==1){			// try to listen at given server socket, if failed 
        ...
        clientSocketsAddress.push_back(clientIP+":"+std::to_string(clientPort));// build client name (key for client socket map) as ip:port and add to client sockets address vector 
	clientSockets[clientSocketsAddress.back()]=CustomSocket(serviceSocket,0,clientIP,clientPort,infoLevel);// make / build client Socket (no variable must be constant in structure)
	if (clientSockets[clientSocketsAddress.back()].state>0){	// if client is connected to server socket 
		//clientSocketThreads.push_back(std::thread(&CustomSocket::coreListener,clientSockets[clientSocketsAddress.back()],BUFFER_SIZE));// make threaded call for core listener and add to client thread socket list #or 
		clientSocketThreads.emplace_back(&CustomSocket::coreListener,clientSockets[clientSocketsAddress.back()],BUFFER_SIZE);// make threaded call for core listener and add to client thread socket list
		//clientSocketThreads.back().join();			// wait till thread ends 
...
void Server::waitTillCommunicationEnds(){				// method to make server wait till all client socket ends 
	for(std::thread& clientSocketThread:clientSocketThreads) clientSocketThread.join();// make wait till thread ends 
}									// end of `waitTillCommunicationEnds` method 


void Server::closeServiceSocket(){					// method to close server (close server socket)
	disconnectAllClients();						// make disconnect all client sockets connected to server 
	serviceSocket.closeSocket();					// make close server socket 
}									// end of `closeServiceSocket` method 


void Server::disconnectAllClients(){					// method to close client socket 
	// iterating over each sockets clientSockets.first -> key , clientSockets.second -> value 
    	for (auto& clientSocket:clientSockets) clientSocket.second.closeSocket();// make close one by one all client sockets 
